...
<html><head>
    ...
    <title>订单详情-美家到家-恩施家政服务预订平台</title>
            ...
            <section class="wrapper order-info" style="margin-top: 12px;">
                ...
                <ul>
                    <li class="clearfix">
                        ...
                        <div class="props">
                            <p class="btn-link name">${order.detailName}</p>
                            <c:if test="${null != order.time && '' != order.time}">
                                    <p>
                                        服务时间：
                                        <fmt:formatDate value="${order.time}" pattern="yyyy-MM-dd" />
                                    </p>
                        	</c:if>
                        	<c:if test="${order.detailId == 1}">
                                <p>数量：${order.peopleNum}人</p>
                        	</c:if>
                        	<c:if test="${order.area == 1}">
                        		<p>服务面积：0-89㎡ </p>
                        	</c:if>
                        	<c:if test="${order.area == 2}">
                        		<p>服务面积：90-159㎡ </p>
                        	</c:if>
                        	<c:if test="${order.area == 3}">
                        		<p>服务面积：160-249㎡ </p>
                        	</c:if>
                        	<c:if test="${order.area == 4}">
                        		<p>服务面积：250-300㎡ </p>
                        	</c:if>
                                <p>
                                    总价：<span class="red" id="unitPrice">￥${order.paymoney}</span> <input type="hidden" id="originPrice" value="${order.paymoney}">
                                </p>
                            <input type="hidden" id="totalPriceHidden" value="${order.paymoney}">
                        </div>
                    </li>
                </ul>
            </section>
...
<%--						  </c:otherwise> --%>
<%--						</c:choose>--%>
                                    <label data-id="${addressInfo.addressId}" class="location">
									<span style="padding-left: 0px;" class="content">
										<span class="title">${addressInfo.name}</span> <br>
										<span class="btn-link" style="padding-top: 6px;">${addressInfo.phone}</span><br>
										<span class="address" style="line-height: 22px;">${addressInfo.address}</span>
									</span>
<%--										<c:choose> --%>
<%--										  <c:when test="${order.state == 0}">   --%>
...
<%--                        <c:choose> --%>
<%--						  <c:when test="${order.state == 0}">   --%>
<%--	                        <li><label id="balance"><span class="content">--%>
<%--						    	<input type="checkbox">使用余额：<span class="orange number">${order.money}.00</span></span>--%>
<%--	                        </label></li>--%>
<%--						  </c:when> --%>
<%--						  <c:otherwise>   --%>
<%--						  </c:otherwise> --%>
<%--						</c:choose>--%>
            ...
            <section class="wrapper order-price">
                <ul class="b">
<%--                    <li class="nb"><label style="padding-bottom: 7px;"> <span class="title">商品总价</span><span class="content serve-price" id="sumMoney">￥${order.money * order.peopleNum}</span></label></li>--%>
<%--                    <li class="nb"><label style="padding-top: 7px; padding-bottom: 7px;"><span class="title">优惠抵用</span><span class="content" id="couponMoney">￥${order.discount}.00</span>--%>
<%--                    </label></li>--%>
<%--                    <li class="nb"><label style="padding-top: 7px;"><span class="title">余额抵用</span><span class="content" id="balanceMoney">￥${order.account}.00</span>--%>
<%--                    </label></li>--%>
                    <li><label><span class="title">总金额 </span><span class="red total-price" id="totalPrice">￥${order.paymoney}</span> </label></li>
                </ul>
                ...
                <aside class="normal">
                    <div id="submitOrder" class="none">
                        <form action="wximpl/orderpay" method="POST">
                            ...
                            <input type="hidden" name="paymoney" value="${order.paymoney}"> 
                            <input type="hidden" name="detailName" value="${order.detailName}">
                    ...
                    <ul>
                        <c:choose> 
						  <c:when test="${order.state == 0}">   
	                        <li><a class="btn orange" href="javascript:void(0);" id="submitBtn">继续支付</a></li>
						  </c:when>
						  <c:when test="${order.state == 1}">   
	                        <li><a class="btn orange" href="javascript:void(0);" >派单中，请稍等</a></li>
						  </c:when>
						  <c:when test="${order.state == 2}">   
	                        <li><a class="btn blue" href="javascript:void(0);" >已派单，阿姨正在路上</a></li>
						  </c:when> 
						  <c:when test="${order.state == 3}">   
	                        <li><a class="btn blue" href="javascript:void(0);" >订单已经完成，感谢您的支持</a></li>
						  </c:when>   
						  <c:otherwise>
						  	<li><a class="btn gray" href="javascript:void(0);">您已经取消该订单</a></li>
						  </c:otherwise> 
						</c:choose>
                    </ul>
...
<script type="text/javascript">
	...
	<script type="text/javascript">
		$('#submitBtn').click(function() {
			$("#submitBtn").addClass('disable').html('<img src="common/images/loading-2.gif"/> 支付中……');
